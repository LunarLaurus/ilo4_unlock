...
# binary generated by ilo4_toolbox
...
while off >= 0:
    ...
    with open(os.path.join(outdir, "%s.hdr" % IMG_LIST[ilo_num]), "wb") as fff:
    ...
    with open(os.path.join(outdir, "%s.sig" % IMG_LIST[ilo_num]), "wb") as fff:
    ...
    with open(os.path.join(outdir, "%s.raw" % IMG_LIST[ilo_num]), "wb") as fff:
        fff.write(data1)

    print("[+] Decompressing")
    output_size = decompress_all(data1, os.path.join(outdir, "%s.bin" % IMG_LIST[ilo_num]))
...
with open(os.path.join(outdir, "firmware.map"), "w") as fff:
